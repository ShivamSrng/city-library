...
class ComputeFine:
  ...
  def execute(self, reader_id: str) -> dict:
    ...
    if reader["query_result"] is None:
      ...
      fine_query = f"""
      SELECT R.RID, R.RNAME, DOC.TITLE, BO.BDTIME, BO.RDTIME, GREATEST(DATEDIFF(BO.RDTIME, BO.BDTIME) - 20, 0) AS ADDITIONAL_DAYS_BORROWED, 0.2 * GREATEST(DATEDIFF(BO.RDTIME, BO.BDTIME) - 20, 0) AS 'FINE_IMPOSED IN $'
      FROM READER AS R, BORROWS AS B, BORROWING AS BO, COPY AS COP, DOCUMENT AS DOC
      WHERE R.RID = '{reader_id}' AND R.RID = B.RID AND B.BOR_NO = BO.BOR_NO AND B.DOCID = COP.DOCID AND B.COPYNO = COP.COPYNO AND B.BID = COP.BID AND COP.DOCID = DOC.DOCID ;
      """
