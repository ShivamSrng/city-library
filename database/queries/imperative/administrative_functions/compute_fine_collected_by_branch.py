...
class ComputeFineCollectedByBranch:
  ...
  def execute(self,
    ...
    query = f"""
    SELECT BRAN.BID, BRAN.BNAME, SUM(GREATEST(DATEDIFF(BORW.RDTIME, BORW.BDTIME) - 20, 0) * 0.2) AS TOTAL_FINE_COLLECTED
    ...
    GROUP BY BRAN.BID
    ORDER BY SUM(GREATEST(DATEDIFF(BORW.RDTIME, BORW.BDTIME) - 20, 0)) DESC;
    """
