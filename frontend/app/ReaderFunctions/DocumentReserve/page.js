...
const documentReserve = ({readerid, readername}) => {
  return (
    <>
      <div className={styles.loginPageContainer}>
        ...
        <form className={styles.formContainer} onSubmit={
              async (event) => {
                ...
                const dynamicapi = 'http://localhost:8000/cityLibrary/reader/reserveDocument/' + readerid + '/' + documentid + '/' + copyno + '/' + branchid
                ...
                if (data.status == "success" && data.hasOwnProperty('descriptive_error') == false){
                  content += "<p>Document Reservation Successful</p>";
